...
namespace autonaming
{
    ...
    {
        if (unit->skills_initial_[skill_name] + 60 < unit->skills_[skill_name])
            return skills_control::get_skill_lvl_from_days(unit->skills_[skill_name]);//for case when skill was modified by giving unit, we should use modified variant
        else
            return skills_control::get_skill_lvl_from_days(unit->skills_initial_[skill_name]);
    }
    ...
    bool check_if_skill_is_increasong(CUnit* unit, const std::string& highest_skill)
    {
        return unit->skills_initial_[highest_skill] < unit->skills_[highest_skill] && 
               unit->skills_initial_[highest_skill] + 60 >= unit->skills_[highest_skill];
    }
    ...
    std::string generate_skill_line(CUnit* unit, const std::string& highest_skill, long highest_lvl)
    {
        ...
        if (highest_lvl > -1)
        {
            ...
            add_skill(skills_line, highest_skill, highest_lvl, check_if_skill_is_increasong(unit, highest_skill));
            for (const auto& skill : sorted_skills)
            {
                ...
                if (skill_type_weights[skills_map[skill.first]] > skill_type_weights["X"])//full skill
                    add_skill(skills_line, skill.first, get_skill_lvl(unit, skill.first), check_if_skill_is_increasong(unit, skill.first));
                else if (skill_type_weights[skills_map[skill.first]] == skill_type_weights["X"])//first letter
                    add_skill(skills_line, std::string(1, skill.first[0]), get_skill_lvl(unit, skill.first), check_if_skill_is_increasong(unit, skill.first));
